...
exports.exportOrdersToSapoExcel = async (req, res) => {
    try {
        ...
        if (orders && orders.length > 0) {
            ...
            supabase
                .from('orders')
                .update({ exported_sapo_at: currentTime })
                .in('id', exportedIds)
                .then(({ error: updateError }) => {
                    if (updateError) console.error("Lỗi cập nhật exported_sapo_at:", updateError);
                });
        }
        // =======================================================
...
// [MỚI] XUẤT EXCEL ĐƠN HÀNG THEO CHUẨN SHOPEE EXPRESS (SPX)
exports.exportOrdersToSPXExcel = async (req, res) => {
    try {
        const { startDate, endDate, ids } = req.query;

        // Hàm làm sạch số điện thoại chuẩn Việt Nam (bỏ số 0 ở đầu nếu có +84)
        const formatSPXPhone = (phone) => {
            if (!phone) return '';
            let cleaned = phone.toString().replace(/\D/g, '');
            if (cleaned.startsWith('84')) cleaned = '0' + cleaned.slice(2);
            else if (!cleaned.startsWith('0')) cleaned = '0' + cleaned;
            return /^0[35789]\d{8}$/.test(cleaned) ? cleaned : '';
        };
        ...
        // Cập nhật thời gian đã xuất file (exported_spx_at) giống Sapo
        if (orders && orders.length > 0) {
            const exportedIds = orders.map(o => o.id);
            const currentTime = new Date().toISOString();
            supabase.from('orders').update({ exported_spx_at: currentTime }).in('id', exportedIds).then();
        }

        // 2. Tạo file Excel chuẩn SPX
        const workbook = new exceljs.Workbook();
        const worksheet = workbook.addWorksheet('Tạo đơn');

        // Khai báo Header đúng chính xác như file template SPX
        worksheet.addRow([
            '*Mã đơn hàng', '*Tên người nhận', '*Số điện thoại', '*Tỉnh/Thành Phố', '*Quận/Huyện', '*Xã/Phường', 
        ...
        let stt = 1;
        orders.forEach(order => {
            // Xử lý tiền COD: Nếu đã thanh toán (Banking) thì COD = 0, ngược lại thì bằng tổng đơn
            const isPrepaid = order.payment_method === 'banking' || (order.note && order.note.toLowerCase().includes('đã thanh toán'));
            const codAmount = isPrepaid ? 0 : Math.round(order.total_amount);
            const isThuCod = codAmount > 0 ? 'Y' : 'N';
            const hinhThucThanhToan = codAmount > 0 ? 'Người gửi trả' : 'Người gửi trả'; // Shop trả cước phí ship cho SPX

            // Xử lý tách địa chỉ (Tỉnh, Huyện, Xã) từ chuỗi address
            let province = '', district = '', ward = '', detailAddress = order.customer_address || '';
            if (order.customer_address) {
                const parts = order.customer_address.split(',').map(s => s.trim());
                if (parts.length >= 4) {
                    province = parts[parts.length - 1]; // Phần tử cuối
                    district = parts[parts.length - 2]; // Kế cuối
                    ward = parts[parts.length - 3];     // Trực thuộc
                    // Các phần còn lại ghép thành địa chỉ chi tiết
                    detailAddress = parts.slice(0, parts.length - 3).join(', ');
                }
            }

            // Gộp tất cả sản phẩm thành một dòng duy nhất cho SPX dễ hiểu
            let productNames = [];
            let totalWeightGrams = 0;
            
            if (order.order_items) {
                order.order_items.forEach(item => {
                    const sku = item.variants?.sku || '';
                    const size = item.variants?.size || '';
                    const color = item.variants?.color || '';
                    const weight = item.variants?.weight || 200; // Mặc định 200g/sp
                    
                    productNames.push(`[${sku}] ${size}-${color} (x${item.quantity})`);
                    totalWeightGrams += weight * item.quantity;
                });
            }
            
            const productNameString = productNames.join(' | ') || 'Quần áo thời trang';
            // Quy đổi tổng Gram sang KG (Tối thiểu 0.1 KG, tối đa 2 chữ số thập phân)
            const totalWeightKg = Math.max(0.1, totalWeightGrams / 1000).toFixed(2);

            worksheet.addRow([
                stt, // *Mã đơn hàng (Để số đếm tự động hoặc mã ORD của web cũng được)
                order.customer_name, // *Tên
                formatSPXPhone(order.customer_phone), // *SĐT
                province, // *Tỉnh
                district, // *Huyện
                ward, // *Xã
                detailAddress, // *Chi tiết
                '', // Lưu ý địa chỉ
                '', // Mã bưu chính
                productNameString, // *Tên SP
                '', // Số lượng (Giao 1 phần)
                '', // Giá tiền (Giao 1 phần)
                totalWeightKg, // *Cân nặng (KG)
                10, // Dài (CM)
                10, // Rộng (CM)
                10, // Cao (CM)
                order.code, // Mã KH (Dùng mã đơn hàng của Web để dễ đối soát)
                Math.round(order.total_amount), // *Giá trị đơn hàng (Khai báo để bảo hiểm)
                'N', // *Giao 1 phần
                'N', // *Cho thử
                'Y', // *Cho xem không thử
                'N', // Phí từ chối
                '', // Số tiền từ chối
                isThuCod, // *Có thu COD không (Y/N)
                codAmount, // Tiền COD
                'N', // Giá trị cao
                hinhThucThanhToan, // *Thanh toán
                order.note ? order.note.replace(/\r?\n|\r/g, ' ') : '', // Lưu ý giao hàng (Lấy note của khách)
                '', // Nhắc nhở COD
                'Đủ điều kiện' // Cột cuối
            ]);
            stt++;
        });

        // Định dạng file xuất về
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', `attachment; filename=SPX_Mass_Orders_${new Date().toISOString().slice(0,10)}.xlsx`);

        await workbook.xlsx.write(res);
        res.end();

    } catch (error) {
        console.error("Export SPX Error:", error);
        res.status(500).json({ success: false, message: 'Lỗi xuất file Excel SPX: ' + error.message });
    }
};